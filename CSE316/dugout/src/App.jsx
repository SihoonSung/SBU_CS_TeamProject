...
function App() {
  ...
  const getThemeClass = (team) => {
    const safe = team?.toLowerCase().replace(/\s/g, '-');
    return safe ? `theme-${safe}` : 'default-theme';
  };

  return (
    <div className={getThemeClass(userProfile?.team)}>
      <nav>
        <div className="nav-container">
          <ul className="navbar">
            ...
            <li className="hideOnMobile"><Link to="/estimate-system">Estimate System</Link></li>
            <li className="hideOnMobile"><Link to="/community">Community</Link></li>
            <li className="hideOnMobile"><Link to="/my-information">My Information</Link></li>
            ...
            <li className="menu-button" onClick={toggleSidebar}>
              <svg xmlns="http://www.w3.org/2000/svg" height="25px" viewBox="0 -1200 960 960" width="25px" fill="white">
      ...
      <ul className="sidebar" style={{ display: sidebarVisible ? 'flex' : 'none' }}>
        ...
        <li><Link to="/estimate-system">Estimate System</Link></li>

...
const Home = () => {
  ...
  useEffect(() => {
    const fetchLiveScores = async () => {
      try {
        const res = await fetch("http://localhost:5000/live-scores");
        ...
        const mapped = data.map((game, idx) => ({
          ...
          homeTeam: {
            name: game.team1,
            score: game.score1,
            logoUrl: teamLogos[game.team1]
          },
          awayTeam: {
            name: game.team2,
            score: game.score2,
            logoUrl: teamLogos[game.team2]
          },
          status: game.status
        }));
          ...
          {liveGames.length > 0 ? (
            <div className="live-games-grid">
              {liveGames.map((game) => (
                <div key={game.id} className="game-score-card">
                  <div className="game-team home-team">
                    <img src={game.homeTeam.logoUrl} alt={`${game.homeTeam.name} logo`} className="team-logo-small" />
                    <span className="team-name">{game.homeTeam.name}</span>
                    <span className="team-score">{game.homeTeam.score}</span>
                  </div>
                  <span className="vs-separator">:</span>
                  <div className="game-team away-team">
                    <span className="team-score">{game.awayTeam.score}</span>
                    <span className="team-name">{game.awayTeam.name}</span>
                    <img src={game.awayTeam.logoUrl} alt={`${game.awayTeam.name} logo`} className="team-logo-small" />
                  </div>
                  <div className="game-status">{game.status}</div>
                </div>
              ))}
